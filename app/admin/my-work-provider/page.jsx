...
export default function MyWorksPage() {
  ...
  useEffect(() => {
    async function fetchProviderWorks() {
      const cookieRes = await fetch("/api/cookies");
      const cookieData = await cookieRes.json();
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEN_BASE_URL}/api/works/provider/${cookieData.id}?page=${page}&limit=${limit}`
      );
    ...
    <div className="p-6">
      ...
      {works.map((w) => (
        <div key={w._id} className="bg-white shadow p-4 rounded mb-3">
          ...
          <p className="mt-2">
            <span className="text-xs bg-blue-100 px-2 py-1 rounded">
              <Link href={`/admin/work-full-detils-number/${w.swrid}`}>
                Full Details
              </Link>
            </span>
          </p>
